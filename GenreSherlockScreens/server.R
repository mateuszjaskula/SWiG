...
shinyServer(function(input, output) {
  ...
  genre_results <- reactive({
      ...
      if(is.null(input$inputfile$name)) {
        ret <- data.frame(R=0,J=0,M=0,H=0,C=0,E=0);
      } else {
  ...
  output$results <- renderUI({
    tags$div(class="hide",
             ...
             tags$label("Muzyka elektroniczna"),
             tags$div(id="electro",
                      class=" progress shiny-html-output"
             )
            )    
  ...
  output$electro <- renderUI({
    tags$div(class = "progress-bar progress-bar-striped", 
             role="progressbar", 
             style= paste("width: ", toString(genre_results()["E"]), "%", sep=""),
             tags$span(paste(toString(genre_results()["E"]), "%", sep=""))
             )
  })
